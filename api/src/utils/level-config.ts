...
export function resolveLevelConfig(raw: unknown): LevelConfig {
  ...
  let tiers: LevelTier[] = Array.isArray(r.tiers) && r.tiers.length > 0
    ? r.tiers
...
export function levelInfo(totalXP: number, cfg: LevelConfig): LevelInfo {
  ...
  if (level >= cfg.cap) {
    return { level: cfg.cap, title: tier.title, color: tier.color, progress: 100, currentXP: 0, requiredXP: 0, totalXP: safeXP }
  }
  ...
  const progress = requiredXP > 0 ? clamp(Math.round((xpInCurrentLevel / requiredXP) * 100), 0, 100) : 0

  return { level, title: tier.title, color: tier.color, progress, currentXP: xpInCurrentLevel, requiredXP, totalXP: safeXP }
}
